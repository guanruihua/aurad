...
export type FieldStatusRecord = Record<
  string,
  {
    errorStatus: boolean
    errorMsg: any
...
export interface UseForm extends ObjectType {
  ...
  error: ObjectType
  setError(params: ObjectType<any>): void
  resetError(names?: string[]): void
  rules: ObjectType<Rule[]>
  /**
   * @description 获取字段值
  ...
  getValues: (
    fieldNames?: string[],
  ) => Record<string, FormFieldValue | FormFieldValue[]>
  /**
   * @description 设单个值
  ...
  clearValues(fieldNames?: string[]): void

...
export function MultipleSelectComponent(props: SelectProps) {
  ...
  const [isLeave, setLeave] = useSetState<Record<string, boolean>>({
    options: true,
  })
  ...
  const Options = (
    <div
      className={classNames('au-select-options', {
        'au-select-options-hover': isHover,
      })}
      ...
      {options?.map((item, index) => {
        ...
        return (
          <div
            ...
            className={classNames('au-select-options-item', {
  ...
  return (
    <div className={classNames(className, { hidden: disabled })}>
      <div
        tabIndex={999}
        className={classNames('au-select-multi-input', 'au-select-input', {
          isHover: isHover && !disabled,
        })}
        onMouseLeave={() => setLeave({ inputBox: true })}
        onClick={() => {
          setLeave({ inputBox: false })
          setHover(true)
        }}
        onBlur={() => {
          !Object.values(isLeave).includes(false) && setHover(false)
